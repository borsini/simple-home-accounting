...
describe(Account.name, () => {
    it('substracts two accounts', () => {
      ...
      expect(result.credits).toEqual(new Decimal(0))
      expect(result.debits).toEqual(new Decimal(0))
      expect(result.children).toEqual(["C2"])
      expect(result.childrenBalance).toEqual(new Decimal(0))
      expect(result.childrenCredits).toEqual(new Decimal(0))
      expect(result.childrenDebits).toEqual(new Decimal(0))
      expect(result.nbChildrenTransactions).toEqual(0)
      expect(result.nbTransactions).toEqual(0)
      expect(result.parent).toEqual("ROOT")
    });
  })
